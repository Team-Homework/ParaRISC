...
**加法器实现备选：分组超前进位加法器（CLA）。** 先用 Chisel 的 `+` 实现 32 位 ALU、AGU 和 PC 加法，以及乘法器末级的进位传播加法，形成可综合、可测量的基线。如果正式 Yosys + ASAP7 时序报告显示某个加法器位于关键路径，再只对该位置尝试分组 CLA，并在相同约束下比较组合延迟、面积及整机频率；乘法器末级的位宽也要按实际实现单独评估。CLA 可能缩短进位传播，却会增加生成/传播逻辑与连线，不能预先认定比 `+` 的综合结果更好。若关键路径主要在 PRF 读选择、旁路或发射控制，优先处理那些路径。时钟频率改善也不会自动提高 IPC，仍需运行同一 Benchmark 验证。
