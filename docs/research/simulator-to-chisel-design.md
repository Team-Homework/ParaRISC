...
| 任务角色 | 主要内容 | 与对方的关键接口 |
| --- | --- | --- |
| A：后端、D-cache 与分支预测 | 重命名、PRF／Busy、RS／发射、ALU／Branch、AGU／LSQ、ROB／提交／恢复、Store Buffer、D-cache、预测器/训练队列及其独立测试 | 接收完整译码包和预测查询；返回预测并发重定向、MDU 请求；D-cache 下级口接统一仲裁 |
| B：前端与平台 | PC／取指／双译码、I-cache、统一内存仲裁／适配、MDU、Top、公共类型代码、独立 ISA 模型、提交对拍工具与评测脚本 | 发完整译码包和预测查询；收预测响应、重定向、MDU 和 D-cache 下级请求，返回事务结果 |
| 协同验收 | 接口契约、跨模块回归、正式性能口径和两份报告 | 连线、提交轨迹、实验配置及数据 |

PRF 分配、数据读写、就绪、唤醒、恢复以及 D-cache/LSU 由 A 在内部闭环处理；预测器、训练及历史恢复归 A；B 负责取指、I-cache 和译码，通过查询/响应接口调用预测器。A 使用译码包与统一内存/MDU 替身，B 使用后端接收器与 D-cache 替身独立验证。B 的参考模型直接解释原始指令，不复用硬件译码。每人各有八个包含测试的工作包，初始预算 A 108 / B 92 相对点（54% / 46%）；实际剩余量按里程碑复核。详细接口和报告责任见分工计划，执行状态记录在 [任务清单](../planning/todo.md)。
