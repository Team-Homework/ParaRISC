...
>
> 本文独立于架构讨论。学完再读 [C++ 模拟器迁移到 Chisel](simulator-to-chisel-design.md)，会更容易理解实现取舍。
...
已有的 C++ 模拟器可继续承担参考模型的角色，但其函数调用次数、容器查询和循环顺序不会自动变成正确的硬件结构。接下来阅读 [模拟器迁移文档](simulator-to-chisel-design.md)，把本文的寄存器、连接、仲裁和握手概念逐项对应到项目模块。
